...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0010000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (int32_t)src1 >> BITS(imm, 4, 0)); // Right Shift Arith immediate
  ...
  // 算术运算（RV32M拓展）：R-type
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = (int32_t)((int32_t)src1 * (int32_t)src2));  // MUL
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (int32_t)(((int64_t)src1 * (int64_t)src2) >> 32));  // MUL High
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulsu  , R, R(rd) = (int32_t)(((int64_t)src1 * (uint64_t)src2) >> 32)); // MUL High(S)(U)
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulu   , R, R(rd) = (uint32_t)(((uint64_t)src1 * (uint64_t)src2) >> 32)); // MUL High(U)
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if ((int32_t)src1 < (int32_t)src2) s->dnpc += imm);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if ((int32_t)src1 >= (int32_t)src2) s->dnpc += imm);
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if (src1 < src2) s->dnpc += imm);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if (src1 >= src2) s->dnpc += imm); 

  // 跳转
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = cpu.pc + imm; R(rd) = s->snpc); // Jump and Link
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = imm + src1; R(rd) = s->snpc); // Jump and Link Reg
